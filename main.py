...
FW_VERSION = "02.00R001 from 06.04.2026"
...
cached_weight = None
#===============Dynamic Messuerements===========
FAST_THRESHOLD = 20   # Gramm → Spule bewegt
fast_mode = False
fast_mode_until = 0
# ===== FILTER HELPERS =====
...
def clamp_step(new, old):
    ...
    return new
# ===== Zusatz Fileter Moving Average ====
_avg_buf = []
_AVG_N = 4 #10

def smooth_weight_ma(val):
    global _avg_buf
    _avg_buf.append(val)
    if len(_avg_buf) > _AVG_N:
        _avg_buf.pop(0)
    return sum(_avg_buf) / len(_avg_buf)
# ==========Exponential Moving Average (EMA)===============
_alpha = 0.4   # 0.1 = sehr smooth, 0.5 = schnell

_ema = None

def smooth_weight(val):
    global _ema
    if _ema is None:
        _ema = val
    else:
...
try:
    while True:
        wdt.feed()
        #===============Dynamic Messueremenst===========
        

        # Gewicht messen und in Median-Puffer schieben
        new_weight = scale.get_grams(20)
        if new_weight is not None:
            m = _median_push(new_weight)
            cached_weight = smooth_weight(m)
